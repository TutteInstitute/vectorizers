...
def read_edge_data(X):
    ...
    if edge_list.shape[1] != 3:
        raise ValueError(f"Incorrect format of data passed in.  "
...
class EdgeListVectorizer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        valid_cols = np.isin(self.edge_list_[:, 1], list(self.column_label_dictionary_.keys()))
    ...
    def transform(self, X):
        ...
        valid_cols = np.isin(edge_list[:, 1], list(self.column_label_dictionary_.keys()))
        valid_edges = valid_rows & valid_cols
        row_indices = [self.row_label_dictionary_[x] for x in edge_list[valid_edges, 0]]
        col_indices = [self.column_label_dictionary_[x] for x in edge_list[valid_edges, 1]]

        matrix = scipy.sparse.coo_matrix((edge_list[valid_edges, 2].astype(float), (row_indices, col_indices))).tocsr()
