...
def test_numpy_vs_list_vs_iisig(truncation_level, log, basepoint, seed=1):
    ...
    np.random.seed(seed)
    test_paths_list = [np.random.normal(size=(path_len,path_dim)) for i in range(n_paths)]
    test_paths_numpy = np.array(test_paths_list)

    sigs_numpy = SignatureVectorizer(truncation_level=truncation_level,
                                     log=log,basepoint=basepoint).fit_transform(test_paths_numpy)
    sigs_list = SignatureVectorizer(truncation_level=truncation_level,
                                     log=log,basepoint=basepoint).fit_transform(test_paths_list)
    ...
    assert np.all(sigs_numpy==sigs_list)
    assert np.all(sigs_list==sigs_iisig)
 

# Check bad initialisation returns appropriate error messages    
def test_bad_init_params():
    with pytest.raises(AssertionError,match='Error: expecting int type for truncation_level.'):
    ...
    with pytest.raises(AssertionError,match='Error: expecting bool type for basepoint'):
...
def test_bad_fit_params():
    ...
    with pytest.raises(AssertionError,match='Error: Expecting numpy array or list of paths.'):
        vectorizer.fit("Not a list or numpy array")
        
    with pytest.raises(AssertionError,match='Error: Expecting list entries to be numpy arrays.'):
...
def test_bad_transform_parameters():
    ...
    with pytest.raises(AssertionError,match='Error: Not all paths share the same dimension.'):
        vectorizer.transform([np.random.random(size=(10,3)),
